...
/**
 * Determine if the current tag represents having walked the total defined length 
 * of bytes for the end of a defined length sequence element, which doesn't have a 
 * distinct delimiting element, it just relies on checking the number of tarversed
 * bytes since encountering the first relevant byte of the sequence against the length 
 * that the SQ instructed us to expect to have to traverse to reach the end. 
 *
 * This is base case detection for use within parse().
 *
 ...
 * running the switch statement that parses values, because the recursive end of a child sq's
 * last element value can also be the end of 1-n parent sequences. 
 *
...
export function parseSQ(buffer: Buffer, ctx: Ctx, el: Element, parentCursor: Cursor) {
  ...
  const partial = parse(item1, ctx); // parse returns 0 length buffer if no elem was truncated
...
export function validateHeader(buffer: Buffer): void | never {
  ...
  if (strAtHeaderPosition !== PRFEFIX) {
    throw new DicomError({
      errorType: DicomErrorType.VALIDATE,
