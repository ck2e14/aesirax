...
import { Element, valueIsTruncated } from "./parse.js";
...
export function decodeTag(buf: Buffer, ctx: Ctx): TagStr {
   ...
   const decode = (offset: number): string => {
      return ctx.usingLE
         ...
         : buf.readUInt16BE(offset).toString(16).padStart(4, "0"); // hexes are base 16 so pass radix 16. pad with 0s to make it 4 chars long
   };
