...
-  Optimize for efficiency during parsing.

### Data Manipulation

-  **Anonymization/Pseudonymization**: Implement in-place byte replacement.
...
1. For traversable and complete datasets:
   a. Use streaming for very large files to parallelize parsing and I/O.
   b. For smaller files, read the whole buffer upfront.
2. For specific end goals, use transform streams to parallelize read I/O, parsing/manipulating, and data emission.
...
1. **SQL Injection (SQLi)**
   ...
   -  Mitigation: Use parameterized queries and proper sanitization.
...
4. **Command Line Injections**
   ...
   -  Risk: Poorly sanitized element values passed to CLI tools.
...
6. **Steganography**
   -  Method: Hide malicious code within DICOM image data.
   -  Leverages DICOM's intrinsic link to imaging formats.
