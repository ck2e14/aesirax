...
function handleSequenceParsing(seqBuffer: Buffer, bundle: StreamBundle, seqTag: string) {
   ...
   const itemDelimTag: TagStr = "(fffe,e00d)" as TagStr; // not sure needed in this fn, its for the parse() basecase, not here
   ...
   if (length === 4_294_967_295) {
      ...
      console.log(
         "alright then, lets check our bundle dataset and hopefully it persisted properly..."
      );
...
function decodeValueLengthAndMoveCursor(
   ...
   if (isUndefinedLengthSQ) {
      ...
      console.log(
         "back from the sequence parsing, walking the parent cursor by",
         bundle.sequenceBytesTraversed
      );
      ...
      console.log(
         "next tag in the buffer is",
         decodeTagNum(buffer.subarray(cursor.pos, cursor.pos + ByteLen.TAG_NUM))
      );
