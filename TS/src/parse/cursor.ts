 ...
 */
let id = -1;
export function newCursor(pos = 0, ctx: Ctx, buf?: Buffer, isOuter = false): Cursor {
   id++;
      ...
      sync(ctx: Ctx, buffer: Buffer) {
         if (ctx.sqStack.length > 1) {
            ctx.sqBytesStack[ctx.sqBytesStack.length - 2] =
               ctx.sqBytesStack[ctx.sqBytesStack.length - 2] + ctx.sqBytesStack[ctx.sqBytesStack.length - 1];
         }
