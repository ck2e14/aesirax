...
export async function multiThreaded(cfg: Global.Cfg) {
  ...
  const nWorkers = cpus().length > dicomFiles.length ? dicomFiles.length : cpus().length; // this could be refined because one massive file also benefi
  // ts from multiple workers but currently doing 1 file per worker. Future improvement.
